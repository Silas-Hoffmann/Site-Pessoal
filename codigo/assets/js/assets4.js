    ...
    function getApiGitHubRep1info() {
        fetch('https://api.github.com/users/Silas-Hoffmann/repos')
            .then(async res => {
                ...
                const createdAt = new Date(data[2].created_at);
                ...
                <section id="repositorio-canvas">
                    <h2 class="titulos">${data[2].name}</h2><!--título-->
                    <hr>
                    <div class="texto">
                        <p><h4>Descrição</h4><br>${data[2].description}</p>
                        <p><h4>Data de Criação</h4><br>${date}</p>
                        <p><h4>Linguagem principal</h4><br>${data[2].language}</p>
                        <p><h4>Link para o repositório</h4><br>${data[2].html_url}</p>
